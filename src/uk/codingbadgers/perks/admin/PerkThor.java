...
public class PerkThor {
	...
	public static void onPlayerInteract(PerkPlayer player,
			...
			if (event.getAction() == Action.LEFT_CLICK_AIR) {
				if (player.getThorAmmount() != 1) {
					for (int i = 0; i < player.getThorAmmount(); i++) {
						Block block = player.getPlayer().getTargetBlock(null,
								300);
						if (block == null)
							return;
						...
						loc.setX(loc.getX()
								+ random.nextDouble()
								* (player.getThorAmmount() > 1 ? player
										.getThorAmmount() / 2 : 1) - 10);
						loc.setZ(loc.getZ()
								+ random.nextDouble()
								* (player.getThorAmmount() > 1 ? player
										.getThorAmmount() / 2 : 1) - 10);

						player.getPlayer().getLocation().getWorld()
								.strikeLightning(loc);
					}
					return;
	...
	public static boolean onCommnad(PerkPlayer player, Command cmd,
			String commandLabel, PerkArgSet args) {
			...
			if (args.size() == 0) {
				...
				if (player.isThorEnabled()) {
					...
					PerkUtils.OutputToPlayer(player,
							"The power of thors hammer has been removed");
					player.getPlayer()
							.getLocation()
							.getWorld()
							.playEffect(player.getPlayer().getLocation(),
					...
					player.setThor(true);
					PerkUtils.OutputToPlayer(player,
							"You have been given thors hammer, use it wisely");
					player.getPlayer()
							.getLocation()
							.getWorld()
							.playEffect(player.getPlayer().getLocation(),
									Effect.MOBSPAWNER_FLAMES,
									new Random().nextInt(9));
					player.setThorHammer(player.getPlayer().getItemInHand()
							.getType());
		...
		if (commandLabel.equalsIgnoreCase("shock")) {
			...
			if (args.size() != 1) {
				...
				if (target == null) {
					PerkUtils.OutputToPlayer(player, args.getString(0)
							+ " is not online");
					return true;
				}
			}
			...
			if (!silent)
				PerkUtils.OutputToAll(player.getPlayer().getName()
						+ " has shocked " + target.getPlayer().getName());

			if (target != player)
				PerkUtils.OutputToPlayer(target, "You have been shocked by "
						+ player.getPlayer().getName());
			PerkUtils.OutputToPlayer(player, "You have shocked "
					+ (target != player ? target.getPlayer().getName()
							: "Yourself"));

			target.getPlayer().getLocation().getWorld()
					.strikeLightning(target.getPlayer().getLocation());
